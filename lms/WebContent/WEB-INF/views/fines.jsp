...
<body>
	...
	<div class="jumbotron text-center">
		<form class="form-inline"  id="finesForm" method="post" >
			<input type="text" id="cardId" class="form-control" size="50" placeholder="Enter Card Details"></input>
...
jQuery(document).ready(function($) {
	...
	$("#finesForm").submit(function(event) {
...
function getFineDetails() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(length==0){
				var msgData='<h2>There are no fines pending for this card ID</h2>';
			...
			for(var i in finesList) {
			    
			        //alert(finesList[i].loanid);
			    	console.log(i);
			        ...
			        var divData= '<div class="list-group-item list-group-item-action">'+
			        				'<div style="float: left; clear:left; margin-right:8px;">'+
			        						'<img src="https://cdn0.iconfinder.com/data/icons/shift-free/32/Currency_dollar-64.png" />'+
			        				'</div>';
			        				
										divData=divData+'<div style="float:right;width:100px;"><a class="myButton" href="payfine?id='+finesList[i].loanid+'">Pay</a></div>';
			        				
			        				divData=divData+'<div style="margin:0 auto;">'+
									'<h3 class="list-group-item-heading">$'+finesList[i].fineAmount+'</h3>'+
									'<p class="list-group-item-text">'+
									
									'<table>'+
									  '<tr>'+
									   ' <td><b>To be paid against Loan Id :</b></td>'+
									   ' <td>'+finesList[i].loanid+'</td>'+
									  '</tr>'+
									'</table>'+
									'</p>'+
									'</div>'+
